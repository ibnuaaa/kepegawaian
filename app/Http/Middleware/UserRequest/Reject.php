...
class Reject extends BaseMiddleware
{
    private function Initiate($request)
    {
        ...
        if ($this->Model->UserRequest) {
            if($this->_Request->input('status_sdm_rejected')) $this->Model->UserRequest->status_sdm = 'rejected';
            if($this->_Request->input('status_diklat_rejected')) $this->Model->UserRequest->status_diklat = 'rejected';
        }
    }
