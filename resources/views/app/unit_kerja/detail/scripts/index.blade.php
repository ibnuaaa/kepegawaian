...
function resetPassword() {
    ...
    axios.post('/unit_kerja/reset_password', {
        unit_kerja_id: "{{$data['id']}}"
    }).then((response) => {
...
function inactiveUnitKerja(status) {

    axios.post('/unit_kerja/change_status', {
        unit_kerja_id: "{{$data['id']}}",
        status: status
    }).then((response) => {
...
function savePLT() {
    ...
    if (g_id > 0) {
    axios.put('/unit_kerja_coupon/' + g_id, {
...
function deletePLT(id) {
    if (confirm('Are you sure ?')) {
        showLoading()
    axios.delete('/unit_kerja_coupon/' + id).then((response) => {
            window.location.reload()
        }).catch((error) => {
            hideLoading()
            if (Boolean(error) && Boolean(error.response) && Boolean(error.response.data) && Boolean(error.response.data.exception) && Boolean(error.response.data.exception.message)) {
...
function editPLT(id) {
    ...
    showLoading()
axios.get('/unit_kerja_coupon/id/' + id + '/set/first').then((response) => {
