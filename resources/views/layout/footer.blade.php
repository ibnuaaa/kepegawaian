    ...
    <script src="/assets/plugins/viewer-js/pdf.js" type="text/javascript"></script>
    ...
    <script>
        ...
        function getNotificationSidebar() {
            ...
            // })
            //
            // //surat_keluar_counter
            // window.axios.get('/mail/with.total/true/for/surat_keluar/status/6').then((response) => {
            //     if(response.data.data.total > 0) {
            //         $("#surat_keluar_counter").html(response.data.data.total);
            //         $("#surat_keluar_counter").removeClass().addClass("show-notification-unread");
            //     } else {
            //         $("#surat_keluar_counter").html("");
            //         $("#surat_keluar_counter").removeClass().addClass("hide-notification");
            //     }
            // })
        }
        ...
        function setPlt() {
            var plt_position_id = $('select[name=plt_position_id]').val();

            showLoading()
            window.axios.put('/user/{{ !empty(MyAccount()->id) ? MyAccount()->id : '0' }}', {
                active_position_id: plt_position_id
            }).then((response) => {
                location.reload()
            })
