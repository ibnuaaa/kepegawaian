    ...
    (function (c, e) {
        var f = {},
            ...
            l = (b.WordArray = y.extend({
                ...
                concat: function (g) {
                    ...
                    g = g.sigBytes;
                    this.clamp();
                    if (k % 4) for (var n = 0; n < g; n++) p[(k + n) >>> 2] |= ((d[n >>> 2] >>> (24 - (n % 4) * 8)) & 255) << (24 - ((k + n) % 4) * 8);
                    else if (65535 < d.length) for (n = 0; n < g; n += 4) p[(k + n) >>> 2] = d[n >>> 2];
            ...
            a = (B.Hex = {
                ...
                parse: function (g) {
                    for (var p = g.length, d = [], k = 0; k < p; k += 2) d[k >>> 3] |= parseInt(g.substr(k, 2), 16) << (24 - (k % 8) * 4);
        ...
        b.Hasher = u.extend({
            ...
            _createHmacHelper: function (g) {
                return function (p, d) {
                    return new w.HMAC.init(g, d).finalize(p);
                };
            },
        });
        var w = (f.algo = {});
...
(function () {
    ...
    c.enc.Base64 = {
        stringify: function (f) {
            ...
            for (var l = 0; l < h; l += 3)
                for (
                    var B = (((b[l >>> 2] >>> (24 - (l % 4) * 8)) & 255) << 16) | (((b[(l + 1) >>> 2] >>> (24 - ((l + 1) % 4) * 8)) & 255) << 8) | ((b[(l + 2) >>> 2] >>> (24 - ((l + 2) % 4) * 8)) & 255), a = 0;
                    4 > a && l + 0.75 * a < h;
                    a++
                )
                    f.push(y.charAt((B >>> (6 * (3 - a))) & 63));
            if ((b = y.charAt(64))) for (; f.length % 4; ) f.push(b);
            return f.join("");
        },
        parse: function (f) {
            var b = f.length,
...
CryptoJS.lib.Cipher ||
    (function (c) {
        ...
        f.BlockCipher = a.extend({
            cfg: a.cfg.extend({ mode: m, padding: u }),
            reset: function () {
                a.reset.call(this);
                var d = this.cfg,
                    k = d.iv;
                ...
                this._mode = n.call(d, this, k && k.words);
            },
        ...
        m = (e.format = {}).OpenSSL = {
            stringify: function (d) {
                var k = d.ciphertext;
                d = d.salt;
                return (d ? h.create([1398893684, 1701076831]).concat(d).concat(k) : k).toString(l);
            },
            parse: function (d) {
                d = l.parse(d);
                var k = d.words;
                ...
                return w.create({ ciphertext: d, salt: n });
            },
        };
        var g = (f.SerializableCipher = b.extend({
            ...
            encrypt: function (d, k, n, x) {
                ...
                k = A.finalize(k);
                A = A.cfg;
                return w.create({ ciphertext: k, key: n, iv: A.iv, algorithm: d, mode: A.mode, padding: A.padding, blockSize: d.blockSize, formatter: x.format });
            },
        ...
        e = (e.kdf = {}).OpenSSL = {
            execute: function (d, k, n, x) {
                x || (x = h.random(8));
                d = B.create({ keySize: k + n }).compute(d, x);
                n = h.create(d.words.slice(k), 4 * n);
                d.sigBytes = 4 * k;
                return w.create({ key: d, iv: n, salt: x });
            },
        };
        var p = (f.PasswordBasedCipher = g.extend({
            cfg: g.cfg.extend({ kdf: e }),
            encrypt: function (d, k, n, x) {
                x = this.cfg.extend(x);
        ...
        _doReset: function () {
            ...
            for (var J = (this._keySchedule = []), H = 0; H < D; H++)
                if (H < C) J[H] = F[H];
                    ...
                    var E = J[H - 1];
                    H % C
                        ? 6 < C && 4 == H % C && (E = (b[E >>> 24] << 24) | (b[(E >>> 16) & 255] << 16) | (b[(E >>> 8) & 255] << 8) | b[E & 255])
                        : ((E = (E << 8) | (E >>> 24)), (E = (b[E >>> 24] << 24) | (b[(E >>> 16) & 255] << 16) | (b[(E >>> 8) & 255] << 8) | b[E & 255]), (E ^= O[(H / C) | 0] << 24));
                    J[H] = J[H - C] ^ E;
                }
            F = this._invKeySchedule = [];
            for (C = 0; C < D; C++) (H = D - C), (E = C % 4 ? J[H] : J[H - 4]), (F[C] = 4 > C || 4 >= H ? E : m[b[E >>> 24]] ^ z[b[(E >>> 16) & 255]] ^ u[b[(E >>> 8) & 255]] ^ w[b[E & 255]]);
        },
        ...
        _doCryptBlock: function (D, F, C, J, H, E, L, q) {
            for (var v = this._nRounds, r = D[F] ^ C[0], t = D[F + 1] ^ C[1], M = D[F + 2] ^ C[2], K = D[F + 3] ^ C[3], N = 4, S = 1; S < v; S++) {
                var P = J[r >>> 24] ^ H[(t >>> 16) & 255] ^ E[(M >>> 8) & 255] ^ L[K & 255] ^ C[N++],
                    Q = J[t >>> 24] ^ H[(M >>> 16) & 255] ^ E[(K >>> 8) & 255] ^ L[r & 255] ^ C[N++],
...
(function (c) {
    ...
    (function () {
        function a(w) {
            for (var g = c.sqrt(w), p = 2; p <= g; p++) if (!(w % p)) return !1;
            return !0;
        }
        function m(w) {
            return (4294967296 * (w - (w | 0))) | 0;
        }
        for (var z = 2, u = 0; 64 > u; ) a(z) && (8 > u && (y[u] = m(c.pow(z, 0.5))), (l[u] = m(c.pow(z, 1 / 3))), u++), z++;
    })();
    var B = [];
    f = f.SHA256 = h.extend({
        _doReset: function () {
        ...
        _doProcessBlock: function (a, m) {
            for (var z = this._hash.words, u = z[0], w = z[1], g = z[2], p = z[3], d = z[4], k = z[5], n = z[6], x = z[7], A = 0; 64 > A; A++) {
                if (16 > A) B[A] = a[m + A] | 0;
                else {
                    var I = B[A - 15],
                        G = B[A - 2];
...
function calElmPos(c, e, f) {
    ...
    return { x1: b + c * Math.cos((Math.PI * y) / e), y1: h + c * Math.sin((Math.PI * y) / e), x2: b + c * Math.cos((Math.PI * f) / e), y2: h + c * Math.sin((Math.PI * f) / e) };
}
function spin(c, e) {
...
function loadRewardBag() {
    try {
        if (localStorage.getItem(cachedKey)) {
            ...
            for (var e = document.querySelector(".reward-list .items"), f = "", b = 0; b < c.length; b++)
                f =
                    !0 === c[b].redeem
                        ? f + ('<div class="item"><button class="btn-redeem disabled"><span>Used</span></button><div class="value" data-value="' + c[b].price + '">' + c[b].price + "</div></div>")
                        : f + ('<div class="item disabled"><button class="btn-redeem" onclick="redeem(' + b + ')"><span>Redeem</span></button><div class="value" data-value="' + c[b].price + '">' + c[b].price + "</div></div>");
            e.innerHTML = f;
...
function turnOnFilters() {
    ...
    if (2 <= _globalVars.config.graphicOption) for (var c = 0; c < _lightOuterMem.length; c++) _lightOuterMem[c].element.filter(setFilter(2, !0));
    if (2 <= _globalVars.config.graphicOption) for (c = 0; c < _globalVars.elms.innerLightArr.length; c++) _globalVars.elms.innerLightArr[c].element.filter(setFilter(1, !1));
    if (1 <= _globalVars.config.graphicOption) for (c = 0; c < _globalVars.elms.textArr.length; c++) _globalVars.elms.textArr[c].filter(setFilter(10, !1));
...
function setFilter(c, e) {
    ...
    e && b.animate({ ease: "<" }).attr({ stdDeviation: "9" }).loop(!0, !0);
        ...
        outerLight: {
            on: function () {
                for (var c, e = 0; e < _lightOuterMem.length; e++)
                    (c = 0.1 * e + "s"),
                        "3" === _globalVars.config.wheelUX ? _lightOuterMem[e].element.animate({ ease: "<", delay: c }).fill("#c9658f").loop(!0, !0) : _lightOuterMem[e].element.animate({ ease: "<", delay: c }).fill("#CD0802").loop(!0, !0);
            },
            off: function () {
...
function drawGraphic() {
    var c = SVG("drawing");
    c.viewbox({ x: 0, y: 0, width: _width, height: _height }).attr("id", "viewBox");
    document.getElementById("viewBox").style.height = window.innerHeight + "px";
    _globalVars = {
        isProcessing: !1,
        coords: { clientX: null, clientY: null },
        config: { wheelUX: "4", totalSlices: 12, distanceDeg: 45, defaultStartDeg: null, borderSlice: 5, sliceWidth: 30, graphicOption: 1, brandLogo: "img/brand.png", backgroundColor: "#337ab7", allowSound: !0 },
        elms: {
            container: c.group(),
            pizza: c.group(),
            pizzaArr: [],
        ...
        jsonData: window._newGlobalVars.jsonData
    };
    _globalVars.config = Object.assign(_globalVars.config, window._newGlobalVars.config)
    if (0 < document.querySelectorAll('[data-type="admin"]').length)
        if ((localStorage.getItem("defaultConfig") && (_globalVars.config = JSON.parse(localStorage.getItem("defaultConfig"))), localStorage.getItem("sliceData"))) _globalVars.jsonData = JSON.parse(localStorage.getItem("sliceData"));
        else {
            if (12 === parseInt(_globalVars.config.totalSlices)) for (var e = 0; e < _globalVars.jsonData.length; e++) _globalVars.jsonData[e].imageUrl = "img/reward" + e + ".png";
        }
    ...
    if ("4" === _globalVars.config.wheelUX) {
        ...
        outerCircle.fill(f);
        f = c.circle("100%").attr({ fill: "#E8DA9F" });
        f.radius("45%");
        outerSpin = c.circle("100%").attr({ fill: "#23051D" });
    ...
    "3" === _globalVars.config.wheelUX &&
        ...
        (outerCircle = c.circle(_width - 30).move(15, 15)),
        outerCircle.fill(f),
        (f = c.circle("100%").attr({ fill: "#ffa000" })),
        f.radius("45%"),
        (outerSpin = c.circle("100%").attr({ fill: "#23051D" })),
    ...
    "4" === _globalVars.config.wheelUX
        ? ((innerSpin = c.circle("100%").attr({ fill: "#501245" })),
                ...
                (e =
                    "3" === _globalVars.config.wheelUX
                        ...
                        : c
                              ...
                              .move(e.x1 - 15, e.y1 - 15)
                              .attr("id", h)),
                _lightOuterMem.push({ element: e, filter: blur }),
    ...
    for (h = 0; h < _globalVars.config.totalSlices; h++) {
        ...
        "4" === _globalVars.config.wheelUX &&
            ...
            e.fill(l),
            e.stroke({ color: "#222222", width: 1, linecap: "round", linejoin: "round" }));
        "3" === _globalVars.config.wheelUX && e.stroke({ color: "#212121", width: 1, linecap: "round", linejoin: "round" });
        "2" === _globalVars.config.wheelUX &&
            ((l =
                0 === h || 0 === h % 2
                    ? c.gradient("linear", function (a) {
            ...
            e.fill(l),
            e.stroke({ color: "#320E34", width: 1, linecap: "round", linejoin: "round" }));
        "1" === _globalVars.config.wheelUX &&
            ((l =
                0 === h || 0 === h % 2
                    ? c.gradient("linear", function (a) {
            ...
            e.fill(l),
            e.stroke({ color: "#E8DA9F", width: 1, linecap: "round", linejoin: "round" }));
        "0" === _globalVars.config.wheelUX &&
            ((l =
                0 === h || 4 === h || 8 === h || 12 === h || 16 === h || 20 === h || 24 === h || 28 === h || 32 === h || 36 === h
                    ? c.gradient("radial", function (a) {
                          a.at(0, "#D80001");
                          a.at(1, "#BC1505");
                      })
        ...
        _globalVars.elms.pizzaArr.push({ element: e, fill: l });
        b = e = e = e = "";
            ...
            for (b = 0; 5 > b; b++)
                (e = _width / 2.5 - 40 * b),
                    ...
                    (e = c
                        ...
                        .move(e.x1 - 8, e.y1 - 8)),
                    e.attr("id", b),
                    _globalVars.elms.innerLightArr.push({ element: e, filter: blur }),
    ...
    for (h = 0; h < _globalVars.config.totalSlices; h++) {
        e = _width / 2.8;
        e = calElmPos(e, _globalVars.config.defaultStartDeg, h);
        36 === parseInt(_globalVars.config.totalSlices) ? ((e.x1 -= 15), (e.y1 -= 13)) : ((e.x1 = e.x1), (e.y1 -= 20), _globalVars.jsonData[h].value.indexOf(" "));
        "4" === _globalVars.config.wheelUX &&
            (l =
                0 === h || 0 === h % 2
                    ? c.gradient("linear", function (a) {
        ...
        if (12 >= parseInt(_globalVars.config.totalSlices))
            if (-1 < _globalVars.jsonData[h].value.indexOf(" ")) {
                ...
                b = c.text(function (a) {
                    for (var m = 0; m < B.length; m++) a.tspan(B[m]).newLine(), a.font({ size: 24, fill: l, anchor: "middle" });
                });
                ...
                b =
                    "3" === _globalVars.config.wheelUX
                        ? c.text(function (a) {
                              a.tspan(_globalVars.jsonData[h].value).font({ size: 22, fill: l, anchor: "middle" });
                          })
                        : c.text(function (a) {
                              a.tspan(_globalVars.jsonData[h].value).font({ size: 30, fill: l, anchor: "middle" });
                          });
        else
            b = c.text(function (a) {
                a.tspan(_globalVars.jsonData[h].value).font({ size: 20, fill: l });
            });
    ...
    shelfTop.fill(l);
    shelfTop.stroke({ color: l, width: 10, linecap: "round", linejoin: "round" });
    ...
    l.from(0, 0).to(0, 1);
    shelfBot.fill(l);
    shelfBot.stroke({ color: l, width: 10, linecap: "round", linejoin: "round" });
    ...
    outerNeedle.fill(l);
    outerNeedle.stroke({ color: l, width: 10, linecap: "round", linejoin: "round" });
    ...
    36 === parseInt(_globalVars.config.totalSlices) && (_globalVars.elms.pizza.rotate(95), _globalVars.elms.innerLight.rotate(95));
    switch (parseInt(_globalVars.config.totalSlices)) {
        case 36:
        ...
        case 10:
            ...
            break;
        case 8:
            _globalVars.elms.rotateGroup.rotate(7), _globalVars.elms.text.rotate(83);
    }
}
...
function decryptedAES(c) {
    ...
    return CryptoJS.AES.decrypt(c, e, { iv: f }).toString(CryptoJS.enc.Utf8);
}
function sendEmail() {
    if (!1 === _globalVars.isProcessing) {
        ...
        Email.send({ Host: "smtp.gmail.com", Username: c, Password: e, To: f, From: c, Subject: b + " You have a winner is waiting for your reward!", Body: b + " Win " + (h ? h[0].price : "$0") }).then(function (y) {
            _globalVars.isProcessing = !1;
