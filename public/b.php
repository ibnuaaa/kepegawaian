...
while ($row=mysqli_fetch_assoc($result))
{
    ...
    $result2 = mysqli_query($link, "SELECT * FROM users where nip='".$row['nip']."'");
    $row2=mysqli_fetch_assoc($result2);
    ...
    if(!empty($row2['nip'])) {
        ...
        $sqll = "insert into users(name,username,nip,password,golongan_id,unit_kerja_id,jabatan_id,position_id) 
            values
            ('".$row['nama']."','".$row['nip']."','".$row['nip']."','$argon2id$v=19$m=1024,t=2,p=2$cjJ0TWhWRjR6ZUxBWVdSTA$ejlomlUBZaNGsaMuYA7AVYqz0YMiE9+X8IQrKuD+Ik0','".$row['golongan_id']."','".$row['unit_kerja_id']."','56','6')
        ";
        // die();
